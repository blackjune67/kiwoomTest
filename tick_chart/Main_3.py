...
class Main(QAxWidget):
    ...
    # GetCommDataEx 한번에 다 조회
    def _comm_get_big_data(self, sTrCode, sRQName, index, item_name):
    ...
    def _opt10080(self, rqname, trcode):
        ...
        # code2 = self._comm_get_big_data(trcode, rqname, 0, "종목코드")
        ...
        for i in range(data_cnt):
            date = self._comm_get_big_data(trcode, rqname, i, "체결시간")
            # inst = self._comm_get_big_data(trcode, rqname, i, "기관계")
            # indi = self._comm_get_big_data(trcode, rqname, i, "개인투자자")
        ...
        df = pd.DataFrame(data[0] + data[1], columns=['현재가', '거래량', '체결시간', 'code','','','','','','','','','',])
